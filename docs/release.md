...
- `artifacts/autoskip-1.1.0.zip`: package with `manifest.json` at the root, no tests/source maps/development code.
- Adjacent `.sha256`: checksum for the exact ZIP.
- `docs/store/images/`: five 1280 × 800 screenshots and one 440 × 280 promotional image.
...
4. Tag a reviewed commit (example for an alpha: `v1.1.0-alpha.1`) and push that exact tag.
