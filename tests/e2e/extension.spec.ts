...
test("installed content script saves a series rule, skips, and really rewinds with Undo", async () => {
  ...
  await expect(
    page.getByRole("button", { name: "Always for this series", exact: true }),
  ).toBeVisible();
  await page
...
test("service rule survives an episode navigation without duplicate clicks", async () => {
  const page = await h.player();
  await page
    .getByRole("button", { name: "Always on this service", exact: true })
    .click();
  await expect.poll(async () => (await h.read()).stats.intros).toBe(1);
  await page.evaluate(() => {
...
test("next episode offers Pause automation and never promises a rewind", async () => {
  const state = structuredClone(DEFAULT_STATE);
...
test("options persist settings and reject malformed imports without changing state", async () => {
  ...
  await page.getByLabel("Extension enabled").uncheck();
  await expect.poll(async () => (await h.read()).enabled).toBe(false);
  await page.reload();
  await expect(page.getByLabel("Extension enabled")).not.toBeChecked();
  await page
    .locator("#import-file")
    .setInputFiles({
      name: "bad.json",
...
test("popup reads the active player and restores series defaults", async () => {
  ...
  await expect(
    popup.locator('[data-scope="series"][data-action="intro"]'),
  ).toBeChecked();
...
test("stale prompt choices after navigation do not change any rules", async () => {
  ...
  await page.evaluate(() => {
    ...
    ).find((b) => b.textContent === "Always on this service");
    button?.click();
  });
  await page.waitForTimeout(1000);
  expect((await h.read()).serviceRules).toEqual({});
